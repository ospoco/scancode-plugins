...
build_p7zip() {
  log "Build p7zip"
  local url="${SEVENZIP_URL:-https://www.7-zip.org/a/7z2501-linux-x64.tar.xz}"
  local archive="$BUILD_ROOT/7zip-linux-x64.tar.xz"
...
patch_libmagic_tree() {
  ...
  patchelf --set-soname libmagic.so "$lib_dir/libmagic.so"
...
patch_p7zip_tree() {
  ...
  if [ -x "$src/7zzs" ]; then
    cp -f "$src/7zzs" "$bin_dir/7z"
  else
    cp -f "$src/7zz" "$bin_dir/7z"
  fi

  for doc in License.txt readme.txt History.txt; do
    if [ -f "$src/$doc" ]; then
      cp -f "$src/$doc" "$license_dir/$doc"
    fi
  done

  chmod 755 "$bin_dir/7z" || true
}
...
patch_xpdf_tree() {
  ...
  mkdir -p "$bin_dir"
  local exe
  exe=$(find "$BUILD_ROOT/xpdf" -type f -name pdftotext -perm -u+x | head -n 1 || true)
  if [ -z "$exe" ]; then
    echo "ERROR: pdftotext not found under $BUILD_ROOT/xpdf" >&2
    exit 1
  fi
  cp -f "$exe" "$bin_dir/pdf2text"
  chmod 755 "$bin_dir/pdf2text" || true
}
...
patch_binary_analysis() {
  ...
  elif [ -x "/usr/bin/universal-ctags" ]; then
    cp -f /usr/bin/universal-ctags "$ctags_bin/ctags"
  else
...
build_wheels() {
  log "Build manylinux2014_x86_64 wheels"
  ...
  for plugin in "${plugins[@]}"; do
    ...
    rm -rf dist build
    "$PYTHON_BIN" setup.py clean --all bdist_wheel --plat-name manylinux2014_x86_64 --python-tag py3
